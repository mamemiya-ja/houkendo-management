...
OneDrive同期を併用する場合は、Microsoft EntraのSPAリダイレクトURIにもGitHub PagesのURLを登録してください。

```text
https://ユーザー名.github.io/リポジトリ名/
```

GitHub PagesのURL自体は外部から見えるため、台帳データはSupabaseまたはOneDriveに置き、リポジトリへ台帳JSONを入れない運用にします。
